...
// AuthGet returns the raw credential JSON for one runtime auth index. Used to
// harvest a catalog-refresh key from credentials that live in auth files
// rather than in plugin config.
func (b *HostBridge) AuthGet(ctx context.Context, authIndex string) (json.RawMessage, error) {
	env, err := b.invoke(ctx, pluginabi.MethodHostAuthGet, pluginapi.HostAuthGetRequest{AuthIndex: authIndex}, "host auth get")
	if err != nil {
		return nil, err
	}
	var resp pluginapi.HostAuthGetResponse
	if len(env.Result) > 0 && json.Unmarshal(env.Result, &resp) != nil {
		return nil, fmt.Errorf("host auth get failed: undecodable response body")
	}
